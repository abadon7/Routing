const getNavButton = ({ id, label, iconPath, currentView, activeView, sidebarCollapsed, currentDesign }) => {
  const isActive = currentView === activeView;
  let stateClass = '';
  
  if (currentDesign === 'tactician') {
    stateClass = isActive 
      ? 'text-slate-900 dark:text-white font-bold' 
      : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all';
  } else {
    stateClass = isActive
  ...
  return `
    <button id="${id}" class="w-full flex items-center ${sidebarCollapsed ? 'justify-center px-2' : 'gap-3 px-4'} py-2.5 rounded-lg text-sm font-medium transition-all relative ${stateClass}" title="${sidebarCollapsed ? label : ''}">
      ${(currentDesign === 'tactician' && isActive) ? '<div class="absolute left-0 top-2 bottom-2 w-1 bg-orange-600 rounded-full"></div>' : ''}
    ...
    <div class="flex h-screen bg-slate-50 dark:bg-slate-900 font-sans overflow-hidden transition-colors duration-300 ${isTactician ? 'tactician-design' : ''}">
      <aside class="${sidebarCollapsed ? 'w-24' : 'w-64'} ${sidebarStyles} flex flex-col hidden md:flex z-20 transition-all duration-300 ${currentDesign === 'foundation' ? 'foundation-sidebar' : ''}">
        ...
        <nav class="flex-1 ${sidebarCollapsed ? 'p-3' : 'p-4'} space-y-1 overflow-y-auto">
            ${sidebarCollapsed ? '' : `<p class="px-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.1em] mb-2 mt-4 ${isTactician ? 'display-font' : ''}">Main</p>`}
            ${getNavButton({ id: 'nav-calendar', label: 'Dashboard', iconPath: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z', currentView, activeView: 'calendar', sidebarCollapsed, currentDesign })}
            ${getNavButton({ id: 'nav-congregations', label: 'Congregations', iconPath: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', currentView, activeView: 'congregations', sidebarCollapsed, currentDesign })}
            ${sidebarCollapsed ? '' : `<p class="px-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.1em] mb-2 mt-4 ${isTactician ? 'display-font' : ''}">Assembly Manager</p>`}
            ${getNavButton({ id: 'nav-assemblies', label: 'Assemblies', iconPath: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10', currentView, activeView: 'assemblies', sidebarCollapsed, currentDesign })}
            ...
            ${getNavButton({ id: 'nav-reports', label: 'Reports', iconPath: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', currentView, activeView: 'reports', sidebarCollapsed, currentDesign })}
        </nav>
